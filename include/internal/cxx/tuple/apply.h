...
namespace std {
  ...
  constexpr decltype(auto) __apply_helper(Fn&& fn, Tuple&& tuple, __index_sequence<Index...>) {
    return fn(get<Index>(std::forward<Tuple>(tuple))...);
  }

  template<typename Fn, typename Tuple>
  constexpr decltype(auto) apply(Fn&& fn, Tuple&& tuple) noexcept {
    return __apply_helper(std::forward<Fn>(fn), std::forward<Tuple>(tuple), __make_index_sequence<__tuple_size<typename __remove_reference<Tuple>::type>::value>());
  }
} // namespace std
